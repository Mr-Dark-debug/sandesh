...
class FolderService:
    ...
    async def get_user_folders(self, user_id: int) -> List[Folder]:
        return await self.folder_repo.get_by_user_id(user_id)

    async def create_folder(self, name: str, user: User) -> Folder:
        existing = await self.folder_repo.get_by_name_and_user(name, user.id)
        if existing:
            raise SandeshError("Folder already exists")

        new_folder = Folder(id=None, name=name, user_id=user.id)
        return await self.folder_repo.save(new_folder)
